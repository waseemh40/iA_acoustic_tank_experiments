function z=fft_comparison_envelop(filename)

[data,Fs]=audioread(filename);
samples=3.3*256000;
samples_pulse=256000;
...
figure
plot(data);
data=data(samples_pulse:max_index);
...
index=1;
    while( index < max_index)
        ...
        if data(index) > (0.015)   
            ...
            upper_limit=index+samples-101;;%samples/8->10000;
            data_extracted(1:+samples,1,set)=data(lower_limit:upper_limit);
            index=index+samples;
            %index
            if set >no_of_sets
                break;
...
peak_f=zeros(no_of_sets,1);
    for index=1:no_of_sets
        [M_mag,P_mag]=max(fft_set((samples/2+1000):samples,1,index));
        peak_f(index,1)=f(samples/2+P_mag+1000);
    end
    peak_f;
